...
(** OCaml output subformat dispatching the output depending on file
    ...
    Format before pprinting the tree. *)
module OCamlPredefSubFormat = struct
  ...
  let pprint col max path data =
    ...
    SF.pprint col !PredefOptions.width path (SF.parse path data)

  let prefix path data =
    let module SF = (val find_subformat path) in
    SF.prefix path (SF.parse path data)

  let ty path data =
    let module SF = (val find_subformat path) in
    SF.ty path (SF.parse path data)

  let name path data =
    let module SF = (val find_subformat path) in
    SF.name path (SF.parse path data)

  let parse path data = data
end
